...
BAUD_RATE = 9600  # Baud rate must match the LoRa configured rate (2.4 kbps = 2400 baud)
    ...
    try:
        ...
        while True:
            ...
            while time.time() - start_time < duration:
                if ser.in_waiting > 0:
                    incoming_message = ser.read(ser.in_waiting) #.decode().strip()  # Read until '\n' and decode to string
                    print(len(incoming_message)) # print(f"Message received: {incoming_message}")
                    bytes_received += len(incoming_message)  # Convert back to bytes for accurate count
                time.sleep(1)      
